# File: res://scripts/systems/TectonicPinholeCleanupCompute.gd
extends RefCounted

const CLEANUP_SHADER_FILE: RDShaderFile = preload("res://shaders/tectonic_pinhole_cleanup.glsl")
...
func _ensure() -> void:
	...
	if not _shader.is_valid():
		var spirv: RDShaderSPIRV = _get_spirv(CLEANUP_SHADER_FILE)
		if spirv != null:
			_shader = _rd.shader_create_from_spirv(spirv)
	if not _pipeline.is_valid() and _shader.is_valid():
		_pipeline = _rd.compute_pipeline_create(_shader)

func cleanup_gpu_buffers(
		w: int,
		h: int,
		height_buf: RID,
		land_buf: RID,
	...
	if w <= 0 or h <= 0:
		return false
	if not height_buf.is_valid() or not land_buf.is_valid() or not boundary_buf.is_valid() or not lava_buf.is_valid():
		return false
	...
	var ints := PackedInt32Array([
		w,
		h,
		max(1, min_land_neighbors),
		max(1, min_boundary_neighbors),
	])
	var floats := PackedFloat32Array([
		sea_level,
		max(0.0001, uplift_amount),
		max(0.0001, max_pit_depth),
	])
	var ints_tail := PackedInt32Array([rng_seed])
	pc.append_array(ints.to_byte_array())
	pc.append_array(floats.to_byte_array())
	pc.append_array(ints_tail.to_byte_array())
