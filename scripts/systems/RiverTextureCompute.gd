# File: res://scripts/systems/RiverTextureCompute.gd
extends RefCounted

# Packs a river mask buffer into an RD texture for GPU-only rendering.

var RIVER_TEX_SHADER: RDShaderFile = load("res://shaders/river_buffer_to_tex.glsl")
...
func _ensure_pipeline() -> void:
	...
	if not _shader.is_valid():
		var spirv := _get_spirv(RIVER_TEX_SHADER)
		if spirv != null:
...
func update_from_buffer(w: int, h: int, river_buf: RID) -> Texture2D:
	_ensure_pipeline()
