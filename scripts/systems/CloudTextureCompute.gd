# File: res://scripts/systems/CloudTextureCompute.gd
extends RefCounted

# Packs a cloud coverage buffer into an RD texture for GPU-only rendering.

var CLOUD_TEX_SHADER: RDShaderFile = load("res://shaders/cloud_buffer_to_tex.glsl")
...
func _ensure_pipeline() -> bool:
	...
	if not _shader.is_valid():
		var spirv := _get_spirv(CLOUD_TEX_SHADER)
		if spirv != null:
...
func _build_uniforms(cloud_buf: RID) -> Array:
	var uniforms: Array = []
	var u := RDUniform.new()
...
func update_from_buffer(w: int, h: int, cloud_buf: RID) -> Texture2D:
	if not _ensure_pipeline():
		return null
	if not cloud_buf.is_valid():
		return null
	...
	if _rd != RenderingServer.get_rendering_device():
		...
		if not _is_tex_rid_usable():
			return null
	var uniforms: Array = _build_uniforms(cloud_buf)
	...
	if not u_set.is_valid():
		...
		if not _ensure_texture(w, h) or not _is_tex_rid_usable():
			return null
		uniforms = _build_uniforms(cloud_buf)
		u_set = _rd.uniform_set_create(uniforms, _shader, 0)
	if not u_set.is_valid():
		push_error("CloudTextureCompute: uniform_set_create failed (w=%d h=%d tex_valid=%s)." % [w, h, str(_is_tex_rid_usable())])
		return null
