...
func compute_to_buffers(
	...
	if w <= 0 or h <= 0:
		return false
	if not height_buf.is_valid() or not land_buf.is_valid() or not distance_buf.is_valid() or not shore_noise_buf.is_valid():
		return false
	if not out_turq_buf.is_valid() or not out_beach_buf.is_valid() or not out_strength_buf.is_valid():
		return false
	var uniforms: Array = []
	var u: RDUniform
	...
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 2; u.add_id(distance_buf); uniforms.append(u)
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 3; u.add_id(shore_noise_buf); uniforms.append(u)
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 4; u.add_id(out_turq_buf); uniforms.append(u)
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 5; u.add_id(out_beach_buf); uniforms.append(u)
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 6; u.add_id(out_strength_buf); uniforms.append(u)
...
func compute(w: int, h: int, height: PackedFloat32Array, is_land: PackedByteArray, sea_level: float, water_distance: PackedFloat32Array, shore_noise_field: PackedFloat32Array, shallow_threshold: float, shore_band: float, wrap_x: bool, noise_x_scale: float) -> Dictionary:
	...
	var ok: bool = compute_to_buffers(
		w,
		h,
		buf_h,
		buf_land,
