...
func _build_uniforms(height_buf: RID, temp_buf: RID, moist_buf: RID, light_buf: RID) -> Array:
	var uniforms: Array = []
	var u := RDUniform.new()
...
func update_from_buffers(w: int, h: int, height_buf: RID, temp_buf: RID, moist_buf: RID, light_buf: RID) -> Texture2D:
	...
	if _rd != RenderingServer.get_rendering_device():
		...
		if not _is_tex_rid_usable():
			return null
	var uniforms: Array = _build_uniforms(height_buf, temp_buf, moist_buf, light_buf)
	...
	if not u_set.is_valid():
		...
		if not _ensure_texture(w, h) or not _is_tex_rid_usable():
			return null
		uniforms = _build_uniforms(height_buf, temp_buf, moist_buf, light_buf)
		u_set = _rd.uniform_set_create(uniforms, _shader, 0)
	if not u_set.is_valid():
		push_error("WorldData1TextureCompute: uniform_set_create failed (w=%d h=%d tex_valid=%s)." % [w, h, str(_is_tex_rid_usable())])
		return null
