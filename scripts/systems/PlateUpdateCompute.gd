...
func build_voronoi_and_boundary(w: int, h: int, site_x: PackedInt32Array, site_y: PackedInt32Array) -> Dictionary:
	_ensure()
	...
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 2; u.add_id(buf_pid); uniforms1.append(u)
	var u_set1 := _rd.uniform_set_create(uniforms1, _label_shader, 0)
	var pc1 := PackedByteArray()
	var ints1 := PackedInt32Array([w, h, site_x.size()])
	...
	_rd.free_rid(buf_sx); _rd.free_rid(buf_sy); _rd.free_rid(buf_pid); _rd.free_rid(buf_pid2); _rd.free_rid(buf_bnd)
...
func apply(
		...
		boundary_mask_u8: PackedByteArray,
	...
	u = RDUniform.new(); u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER; u.binding = 5; u.add_id(buf_height_out); uniforms.append(u)
