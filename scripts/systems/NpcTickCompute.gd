...
func bind_buffers(needs_vec4: RID, local_mask_u32: RID) -> bool:
	if not _ensure():
		return false
	var arr: Array[RID] = [needs_vec4, local_mask_u32]
	for r in arr:
		if not (r is RID and r.is_valid()):
			push_error("NpcTickCompute: missing buffer binding.")
			return false
	_buffers = arr
...
func dispatch(
	...
	abs_day: int,
	dt_days: float,
	need_gain_scale: float = 1.0,
	local_relief_scale: float = 1.0,
	remote_stress_scale: float = 1.0
) -> bool:
	if not _ensure():
		return false
	if not _uniform_set.is_valid():
		push_error("NpcTickCompute: uniform set not bound.")
		return false
	var n: int = max(0, int(npc_count))
	if n <= 0:
		return true
	var pc := PackedByteArray()
	var ints := PackedInt32Array([n, int(abs_day), 0, 0])
	pc.append_array(ints.to_byte_array())
	var floats := PackedFloat32Array([
		float(dt_days),
		clamp(float(need_gain_scale), 0.1, 4.0),
		clamp(float(local_relief_scale), 0.1, 4.0),
		clamp(float(remote_stress_scale), 0.1, 4.0),
	])
	pc.append_array(floats.to_byte_array())
	if not ComputeShaderBase.validate_push_constant_size(pc, 32, "NpcTickCompute"):
		return false

	var groups_x: int = int(ceil(float(n) / 64.0))
