...
func bind_buffers(unrest: RID) -> bool:
	if not _ensure():
		return false
	if not (unrest is RID and unrest.is_valid()):
		push_error("PoliticsTickCompute: missing buffer binding.")
		return false
	_buffers = [unrest]
...
func dispatch(
	...
	abs_day: int,
	dt_days: float,
	unrest_decay_scale: float = 1.0,
	unrest_drift: float = 0.0
) -> bool:
	if not _ensure():
		return false
	if not _uniform_set.is_valid():
		push_error("PoliticsTickCompute: uniform set not bound.")
		return false
	var n: int = max(0, int(province_count))
	if n <= 0:
		return true
	...
	var floats := PackedFloat32Array([
		float(dt_days),
		clamp(float(unrest_decay_scale), 0.1, 4.0),
		clamp(float(unrest_drift), -0.01, 0.01),
		0.0,
	])
	pc.append_array(floats.to_byte_array())
	if not ComputeShaderBase.validate_push_constant_size(pc, 32, "PoliticsTickCompute"):
		return false
