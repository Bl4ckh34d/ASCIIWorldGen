...
func bind_buffers(pop_f32: RID, wildlife_f32: RID, meta_f32: RID) -> bool:
	if not _ensure():
		return false
	var arr: Array[RID] = [pop_f32, wildlife_f32, meta_f32]
	for r in arr:
		if not (r is RID and r.is_valid()):
			push_error("CivilizationTickCompute: missing buffer binding.")
			return false
	_buffers = arr
...
func dispatch(world_w: int, world_h: int, abs_day: int, dt_days: float, emergence_day: int, start_x: int, start_y: int) -> bool:
	if not _ensure():
		return false
	if not _uniform_set.is_valid():
		push_error("CivilizationTickCompute: uniform set not bound.")
		return false
	...
	var ints := PackedInt32Array([w, h, int(abs_day), int(emergence_day), int(start_x), int(start_y), 0, 0])
	pc.append_array(ints.to_byte_array())
	var floats := PackedFloat32Array([float(dt_days), 0.0, 0.0, 0.0])
	pc.append_array(floats.to_byte_array())
	if not ComputeShaderBase.validate_push_constant_size(pc, 48, "CivilizationTickCompute"):
		return false
