...
func bind_buffers(pop_in: RID, wild_f32: RID, pop_out: RID) -> bool:
	if not _ensure():
		return false
	var arr: Array[RID] = [pop_in, wild_f32, pop_out]
	for r in arr:
		if not (r is RID and r.is_valid()):
			push_error("PopulationMigrateCompute: missing buffer binding.")
			return false
	_buffers = arr
...
func dispatch(world_w: int, world_h: int, abs_day: int, dt_days: float, move_rate: float = 0.06, wildlife_low: float = 0.18) -> bool:
	if not _ensure():
		return false
	if not _uniform_set.is_valid():
		push_error("PopulationMigrateCompute: uniform set not bound.")
		return false
	...
	var floats := PackedFloat32Array([float(dt_days), float(move_rate), float(wildlife_low), 0.0])
	pc.append_array(floats.to_byte_array())
	if not ComputeShaderBase.validate_push_constant_size(pc, 32, "PopulationMigrateCompute"):
		return false
