# File: res://scripts/systems/BiomeTextureCompute.gd
extends RefCounted

# Packs biome id buffer into an RD texture for GPU-only rendering.

var BIOME_TEX_SHADER: RDShaderFile = load("res://shaders/biome_buffer_to_tex.glsl")
...
func _ensure_pipeline() -> void:
	...
	if not _shader.is_valid():
		var spirv := _get_spirv(BIOME_TEX_SHADER)
		if spirv != null:
...
func update_from_buffer(w: int, h: int, biome_buf: RID) -> Texture2D:
	_ensure_pipeline()
	if not _pipeline.is_valid():
		return null
	if not biome_buf.is_valid():
		return null
	_ensure_texture(w, h)
	if _tex == null:
		return null
